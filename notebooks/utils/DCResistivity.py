...
from SimPEG.electromagnetics.static.utils import (
    genTopography, gen_DCIPsurvey, plot_1d_layer_model
)
...
from discretize.utils import mesh_utils as meshutils
...
class DC1D3LayerApp(object):
    ...
    def plot_resistivity(self, hz, rho, ax=None, **kwargs):
        ...
        plot_1d_layer_model(mesh_1d.h[0], rho, ax=ax, **kwargs)
    ...
    def simulate(self, hz, rho):
        ...
        for src in self.survey.source_list:
            for rx in src.receiver_list:
                rx.data_type = "apparent_resistivity"
        ...
        ab_locs = self.data.survey.locations_a-self.data.survey.locations_b
        ab = np.sqrt((ab_locs**2).sum(axis=1))
...
class DC1DInversionApp(object):
    ...
    def get_simulation(self):
        ...
        simulation = DC.Simulation1DLayers(
            rhoMap=mapping,
            thicknesses=self.mesh_1d.h[0][:-1],
            Solver=Pardiso,
            survey=self.survey,
            # data_type="apparent_resistivity"
        )
    ...
    def plot_obs_data(self, plot_type, aspect_ratio):
        ...
        ab_locs = self.data.survey.locations_a-self.data.survey.locations_b
        ab2 = np.sqrt((ab_locs**2).sum(axis=1))
    ...
    def plot_model_doi(
        ...
        try:
            ...
            ax2 = axs[1]
            plot_1d_layer_model(
                self.mesh_1d.h[0], rho1, ax=ax1, **{'color': 'k'}
            )
            plot_1d_layer_model(
                self.mesh_1d.h[0], rho2, ax=ax1, **{'color': 'grey', 'linestyle': '--'}
            )
            ax1.set_xlim(vmin, vmax)
            plot_1d_layer_model(
                self.mesh_1d.h[0], doi_index, xscale='linear', ax=ax2
            )
